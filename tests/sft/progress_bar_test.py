...
class ProgressBarTest(absltest.TestCase):
  ...
  def test_update_metric(self):
    self.metrics_logger.log(self.metrics_prefix, "loss", np.array(0.5), metrics_logger.Mode.TRAIN, 1)
    self.metrics_logger.log(self.metrics_prefix, "loss", np.array(0.6), metrics_logger.Mode.EVAL, 1)
    ...
    self.assertDictEqual(
        self.progress_bar.metrics, {"test_train_loss": 0.5, "test_eval_loss": 0.6}
    )

  def test_update_metrics(self):
    # update `metrics_logger`.
    self.metrics_logger.log(self.metrics_prefix, "loss", np.array(0.8), metrics_logger.Mode.TRAIN, 2)
    self.metrics_logger.log(self.metrics_prefix, "loss", np.array(0.9), metrics_logger.Mode.EVAL, 2)
    self.metrics_logger.log(
        self.metrics_prefix, "perplexity", np.array(2.2255), metrics_logger.Mode.TRAIN, 2
    )
    self.metrics_logger.log(
        self.metrics_prefix, "perplexity", np.array(2.4596), metrics_logger.Mode.EVAL, 2
    )
